...
| JournalEntry | Заметки ДМ, разложенные по папкам: `<модуль>/<компендиум>/<папки модуля>` |
...
**Повторный импорт.** Карточки (предметы, заклинания, существа, справочник,
состояния) заводятся заново — дубли по ним не проверяются. А вот заметки
сверяются: если запись с тем же названием в той же папке уже есть и текст
совпадает — она молча пропускается, а если текст разошёлся — импорт
спрашивает, что делать (перезаписать, оставить обе, пропустить), с галочкой
«так же поступать со всеми остальными». Файлы из архива тоже переносятся один
раз: повторный импорт узнаёт их и оставляет прежние ссылки.

...
test("parseCell: режимы и суффикс за уровень", () => {
  ...
  assert.deepEqual(parseCell("/2"), { mode: "div", value: "2", perLevel: false });
  assert.deepEqual(parseCell("×½"), { mode: "div", value: "2", perLevel: false });
  assert.ok(parseCell("/1").bad, "делитель от 2");
...
test("cellText: обратная запись", () => {
  ...
  assert.equal(cellText({ mode: "div", value: "2" }), "×½");
});

...
// dmResetResponse — POST /api/dm-password-reset с заданным адресом клиента.
func dmResetResponse(t *testing.T, api *API, remoteAddr string) (int, map[string]string) {
	t.Helper()
	req := httptest.NewRequest(http.MethodPost, "/api/dm-password-reset", nil)
	req.RemoteAddr = remoteAddr
	rec := httptest.NewRecorder()
...
func TestDMPasswordResetLoopback(t *testing.T) {
	...
	code, body := dmResetResponse(t, api, "127.0.0.1:54321")
	if code != http.StatusOK {
		t.Fatalf("код %d, ожидался 200", code)
	}
...
func TestDMPasswordResetForbidden(t *testing.T) {
	...
	if code, _ := dmResetResponse(t, remote, "192.168.1.15:54321"); code != http.StatusForbidden {
		t.Fatalf("из сети: код %d, ожидался 403", code)
	}

	proxied, proxiedCalls := apiWithReset()
	proxied.SecureCookies = true
	if code, _ := dmResetResponse(t, proxied, "127.0.0.1:54321"); code != http.StatusForbidden {
		t.Fatalf("за прокси: код %d, ожидался 403", code)
	}

	demo, demoCalls := apiWithReset()
	demo.DemoMode = true
	if code, _ := dmResetResponse(t, demo, "127.0.0.1:54321"); code != http.StatusForbidden {

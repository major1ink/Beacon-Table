...
type Monster struct {
	...
	// «Призыва животных». Весь бестиарий игроку не открывается — спойлеры
	// модуля, — поэтому флаг на карточке. Для карточек «из коробки» флаг
	// не хранится (они не редактируются) — их открывает общий тумблер стола
	// CombatState.SummonBuiltin.

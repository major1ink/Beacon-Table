...
type Token struct {
	...
	// killCombatant и handleSetCombatantDeathSave). web/src/vtt/
	...
	// когда этот токен умер (см. service.Room.killCombatant) — с
	...
	// XP — опыт за победу над этим монстром по правилам системы (см.
	// CombatRules.XPFor), снятый в тот же момент и по той же причине, что и
	// Loot выше (service.Room.killCombatant) — карточка монстра
	...
	// убили. 0 — система опыт не считает, поле карточки пустое, либо это
	// голый NPC-токен без MonsterID (опыт за него ДМ прикидывает сам).

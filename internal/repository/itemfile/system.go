// system.go — карточки предметов из модулей (SystemStore, только чтение) и
...
var kind = cardcatalog.Kind[domain.Item]{
	SetCatalog: func(c *domain.Item, id, moduleID string) {
		c.ID, c.System, c.Module = id, true, moduleID
	},
	SetLibrary: func(c *domain.Item) {
		c.System, c.Module = false, ""
	},
	Less: func(a, b *domain.Item) bool {
		return strings.ToLower(a.Name) < strings.ToLower(b.Name)
	},
}

// SystemStore — карточки одного модуля.
type SystemStore = cardcatalog.Source[domain.Item]

// Catalog — библиотека мира плюс карточки подключённых модулей.
type Catalog = cardcatalog.Catalog[domain.Item]
...
func NewCatalog(user *Store, sources ...*SystemStore) *Catalog {
	return cardcatalog.New[domain.Item](user, kind, sources...)
}

...
// Рисовать может только тот, кому доску открыли на правку: чтения мало.
func TestBoardSaveSceneRespectsAccess(t *testing.T) {
	ctx := context.Background()
	s := boardsSvc(t)
	...
	scene := &excalidraw.Scene{
		Type:     "excalidraw",
		Version:  2,
		Elements: []*excalidraw.Element{{ID: "r1", Type: excalidraw.TypeRectangle, Width: 10, Height: 10}},
	}

	// Автор.
	if err := s.SaveScene(ctx, gwen, b.ID, scene); err != nil {
		t.Fatalf("автор не смог записать холст: %v", err)
	}
	// Чужому доски не существует.
	if err := s.SaveScene(ctx, tom, b.ID, scene); err != domain.ErrNotFound {
		t.Errorf("SaveScene чужой доски = %v, ожидался ErrNotFound", err)
	}
	// Чтение: смотреть можно, писать нельзя.
	if _, err := s.SetAccess(ctx, gwen, b.ID, domain.JournalObserver, nil); err != nil {
		t.Fatal(err)
	}
	if _, err := s.Scene(ctx, tom, b.ID); err != nil {
		t.Fatalf("observer не видит холст: %v", err)
	}
	if err := s.SaveScene(ctx, tom, b.ID, scene); err != domain.ErrForbidden {
		t.Errorf("SaveScene при observer = %v, ожидался ErrForbidden", err)
	}
	// Правка.
	if _, err := s.SetAccess(ctx, gwen, b.ID, domain.JournalOwner, nil); err != nil {
		t.Fatal(err)
	}
	if err := s.SaveScene(ctx, tom, b.ID, scene); err != nil {
		t.Errorf("SaveScene при owner: %v", err)
	}
}

// В ручку записи может прилететь произвольный JSON.
func TestBoardSaveSceneRejectsForeignJSON(t *testing.T) {
	ctx := context.Background()
	s := boardsSvc(t)
	...
	if err := s.SaveScene(ctx, gwen, b.ID, &excalidraw.Scene{Type: "tldraw"}); err == nil {
		t.Error("SaveScene приняла не-excalidraw сцену")
	}
	if err := s.SaveScene(ctx, gwen, b.ID, nil); err == nil {
		t.Error("SaveScene приняла nil")
	}
}

// Запись холста не трогает шапку: имя, автор и права на месте.
func TestBoardSaveSceneKeepsHeader(t *testing.T) {
	ctx := context.Background()
	s := boardsSvc(t)

	b, err := s.Create(ctx, gwen, BoardDraft{Name: "Схема", Default: domain.JournalObserver})
	if err != nil {
		t.Fatal(err)
	}
	scene := &excalidraw.Scene{Type: "excalidraw", Version: 2, Elements: []*excalidraw.Element{{ID: "r1", Type: excalidraw.TypeRectangle}}}
	if err := s.SaveScene(ctx, gwen, b.ID, scene); err != nil {
		t.Fatal(err)
	}
	after, err := s.Get(ctx, gwen, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	if after.Name != "Схема" || after.OwnerID != gwen.ID || after.Default != domain.JournalObserver {
		t.Errorf("шапка изменилась после записи холста: %+v", after)
	}
	doc, err := s.Scene(ctx, gwen, b.ID)
	if err != nil {
		t.Fatal(err)
	...
	if len(doc.Scene.Elements) != 1 || doc.Scene.Elements[0].ID != "r1" {
		t.Errorf("холст не сохранился: %+v", doc.Scene.Elements)
	}
}

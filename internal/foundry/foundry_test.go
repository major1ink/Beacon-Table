...
func TestRewriteRolls(t *testing.T) {
	cases := []struct {
		...
		// Регрессия: PHB-2024 пишет обогатитель со строчной буквы, и такой
		// макрос уезжал в описание карточки как есть (заклинание «Сон»).
		{
			"имя обогатителя в любом регистре",
			`с Невосприимчивостью к &reference[exhaustion apply=false]{Утомлению} проходят`,
			`с Невосприимчивостью к Утомлению проходят`,
		},
		{"строчный обогатитель без подписи", `состояние &reference[condition=prone]`, `состояние prone`},
		{"без подписи служебные параметры в текст не идут", `к &reference[exhaustion apply=false] иммунитет`, `к exhaustion иммунитет`},

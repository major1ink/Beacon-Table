...
# проблемы, но журнал растёт быстро.`,
	...
	{
		group: `# Файл журнала. Пусто — beacon.log в каталоге данных; "off" — не писать
# в файл (в Docker и под systemd журнал собирают снаружи).
# Файл нужен прежде всего тем, кто запускает программу двойным кликом: окна
# консоли при этом нет, а в журнале лежит и временный пароль ДМ, и причина,
# по которой сервер не поднялся.`,
		key: envLogFile, example: "data/beacon.log",
		section: "Пути и порт", title: "Файл журнала", hint: "Пусто — beacon.log в каталоге данных, off — не писать в файл.",
		kind: "text", readOnly: true,
	},
	{
		group: `# Открывать ли стол в браузере сразу после запуска: auto, true, false.
# auto — открывать, когда это похоже на запуск за своим компьютером (есть
# графический сеанс). На сервере и в контейнере ничего не открывается.`,
		key: envOpenBrowser, example: "auto",
		section: "Запуск", title: "Открывать браузер при запуске", hint: "auto — только при запуске за своим компьютером. Применится при следующем запуске.",
		kind: "enum", options: []string{"auto", "true", "false"},
	},
	{
...
# удаляется само.`,
	...
	{
		group: `# Публичное демо: витрина, куда пускают без регистрации. Гость выбирает,
# с какой стороны ширмы сесть. Ведущим — права ДМ ВНУТРИ стола (двигать
# токены, ставить свет, править бестиарий); игроком — свой персонаж, своя
# фишка на карте, туман войны и свет глазами игрока. Сервером гость не
# распоряжается ни в той, ни в другой роли — ни аккаунтами, ни мирами, ни
# этими настройками. Стол один на всех: посетители видят друг друга.
#
...
# стол возвращается при сбросе. Без него демо не поднимется.`,
	...
	{
		group: `# Чат за столом: сколько последних сообщений хранить между перезапусками.
# 0 — не хранить вовсе: переписка живёт, пока запущен сервер.`,
		key: envChatHistory, example: "500",
		section: "Чат за столом", title: "Сколько сообщений хранить", hint: "Старые удаляются. 0 — не хранить: чат живёт, пока запущен сервер.",
		kind: "int", appliesNow: true,
	},
}

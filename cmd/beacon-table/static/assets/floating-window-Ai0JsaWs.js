import{a as N}from"./modal-DKhtjNqL.js";import{a as T,c as B}from"./back-stack-C8cwVhaZ.js";let S=!1;function j(){if(S)return;S=!0;const n=document.createElement("style");n.textContent=`
    .fw-window {
  ...
  `,document.head.appendChild(n)}const H=200;let F=H,I=0;const p=new Map;function h(n){F+=1,n.style.zIndex=String(F)}function O({key:n,title:r,url:i,navigate:s=!1,popoutFeatures:l="width=1040,height=880",width:k=1040,height:z=880}){j();const w=p.get(n);if(w){if(s){const e=w.querySelector(".fw-iframe"),a=e.contentWindow&&e.contentWindow.location.href||e.src;new URL(a,location.href).href!==new URL(i,location.href).href&&(e.src=i)}return h(w),w}const y=Math.min(k,Math.round(window.innerWidth*.94)),v=Math.min(z,Math.round(window.innerHeight*.9)),W=I++%8*28,t=document.createElement("div");t.className="fw-window",t.style.width=y+"px",t.style.height=v+"px",t.style.left=Math.max(8,Math.round((window.innerWidth-y)/2)+W)+"px",t.style.top=Math.max(8,Math.round((window.innerHeight-v)/2)+W)+"px";const f=document.createElement("div");f.className="fw-titlebar";const m=document.createElement("span");m.className="fw-title",m.textContent=r||"";const c=document.createElement("button");c.type="button",c.className="fw-btn fw-popout",c.textContent="🗗",c.title="Открыть в отдельном окне браузера";const d=document.createElement("button");d.type="button",d.className="fw-btn",d.textContent="✕",d.title="Закрыть",f.append(m,c,d);const x=document.createElement("div");x.className="fw-body";const o=document.createElement("iframe");o.className="fw-iframe",o.allowFullscreen=!0,o.src=i,x.appendChild(o),t.append(f,x),document.body.appendChild(t),p.set(n,t),h(t);async function M(){let e;try{e=o.contentWindow&&o.contentWindow.beaconFlush}catch{return}if(typeof e=="function")try{await Promise.race([Promise.resolve(e()),new Promise(a=>setTimeout(a,3e3))])}catch{}}const b=B(async()=>{await g()?u():b.set(!0)});b.set(!0);async function u(){b.set(!1),await M(),window.removeEventListener("message",E),t.remove(),p.delete(n)}async function g(){let e="";try{const a=o.contentWindow&&o.contentWindow.beaconCloseGuard;typeof a=="function"&&(e=a()||"")}catch{}return e?N(e,{title:"Закрыть окно",okLabel:"Закрыть",cancelLabel:"Не закрывать",danger:!0}):!0}d.onclick=async()=>{await g()&&u()},c.onclick=async()=>{await g()&&(await M(),window.open(i,n,l),u())};function E(e){e.source!==o.contentWindow||!e.data||(e.data.type==="beacon:closeFloatingWindow"&&u(),e.data.type==="beacon:ownHeader"&&t.classList.add("fw-own-header"))}window.addEventListener("message",E),f.addEventListener("pointerdown",()=>h(t));let L=0,C=0;return T(f,{onStart:()=>{L=t.offsetLeft,C=t.offsetTop},onMove:(e,a)=>{t.style.left=Math.max(0,L+e)+"px",t.style.top=Math.max(0,C+a)+"px"}}),o.addEventListener("load",()=>{try{o.contentWindow.addEventListener("mousedown",()=>h(t),!0)}catch{}}),t}function q(n){return p.has(n)}function R(n,r){for(const[i,s]of p){if(!i.startsWith(n))continue;const l=s.querySelector(".fw-iframe");l&&l.contentWindow&&l.contentWindow.postMessage(r,location.origin)}}function A({key:n,title:r,url:i,navigate:s=!1}){if(window.parent&&window.parent!==window){window.parent.postMessage({type:"beacon:openFloatingWindow",key:n,title:r,url:i,navigate:s},location.origin);return}O({key:n,title:r,url:i,navigate:s})}export{O as a,q as i,A as o,R as p};

import{i as N,j as $}from"./icons-CsEof5Z3.js";import{l as P,d as j}from"./modifier-editor-DrBZj5-6.js";function U(e,n){const o=String(e||"").trim().replace(/\s+/g,"").replace(/[‒–—―−]/g,"-");if(!o)return null;if(/^[+-]\d+$/.test(o)){const s=parseInt(o,10);return{delta:s,value:n+s}}return/^\d+$/.test(o)?{delta:null,value:parseInt(o,10)}:null}function V(e,{getState:n,onPreview:o,onCommit:s}){e&&e.addEventListener("pointerdown",i=>{if(i.button!==0)return;const{current:t,max:r}=n();if(!(r>0))return;i.preventDefault();const m=e.getBoundingClientRect(),d=u=>{const a=Math.max(0,Math.min(1,(u-m.left)/m.width));return Math.round(a*r)};let v=d(i.clientX);o(v);const h=u=>{const a=d(u.clientX);a!==v&&(v=a,o(v))},g=()=>{window.removeEventListener("pointermove",h),window.removeEventListener("pointerup",g),window.removeEventListener("pointercancel",g),v!==t?s(v):o(null)};window.addEventListener("pointermove",h),window.addEventListener("pointerup",g),window.addEventListener("pointercancel",g)})}function W({current:e,temp:n,max:o}){if(!(o>0))return{hp:0,temp:0};const s=Math.max(0,Math.min(1,(e||0)/o)),i=Math.max(0,Math.min(1-s,(n||0)/o));return{hp:s,temp:i}}function X(e){return e>.5?"var(--green-bright, #5fd08a)":e>.25?"var(--gold, #e0c95a)":"#d9534f"}const _=`
.status-palette {

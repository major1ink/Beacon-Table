...
`;let S=!1;function L(){if(S)return;S=!0;const e=document.createElement("style");e.textContent=j,document.head.appendChild(e)}let y=null,C=null;async function _(){return y||(C||(C=I().then(e=>(y=Array.isArray(e)?e:[],y)).catch(()=>[]).finally(()=>{C=null})),C)}function H(){y=null}window.addEventListener("message",e=>{e.data&&e.data.type==="beacon:conditionSaved"&&H()});function M(e,a){if(e&&e.imageUrl){const s=document.createElement("img");return s.src=e.imageUrl,s.alt=a||"",s}const o=document.createElement("span");return o.textContent=e&&e.icon||"❔",o}function A(e){return e.tokenId?{tokenId:e.tokenId}:{combatantId:e.combatantId}}function B(e){return Array.isArray(e.tokenIds)?e.tokenIds.map(a=>({tokenId:a})):[A(e)]}function w(e,a,o,s){for(const l of B(a))e({type:o,...l,...s})}function q(e,{onAdd:a,onRemove:o,addTitle:s}={}){L();const l=document.createElement("div");l.className="status-chips";for(const n of e||[]){const r=document.createElement("span");r.className="status-chip"+(o?" clickable":""),n.color&&r.style.setProperty("--chip-color",n.color),r.appendChild(M(n,n.name));const d=document.createElement("span");d.textContent=n.name+(n.level?` ${n.level}`:"")+(n.rounds?` · ${n.rounds}р`:""),r.appendChild(d);const p=[n.source,n.hidden?"только для ДМ":""].filter(Boolean);r.title=p.length?`${n.name} (${p.join("; ")})`:n.name,n.hidden&&(r.style.opacity="0.6"),o&&(r.onclick=()=>o(n)),l.appendChild(r)}if(a){const n=document.createElement("button");n.type="button",n.className="status-chip-add",n.innerHTML=N("plus",{size:12}),n.title=s||"Наложить состояние",n.onclick=r=>{r.stopPropagation(),a(r)},l.appendChild(n)}return l}let v=null;function k(){v&&(v.el.remove(),v=null)}function Q(){v&&E(v)}async function G({x:e,y:a,target:o,send:s,statusesFor:l,title:n}){L(),k();const r=document.createElement("div");r.className="status-palette",document.body.appendChild(r),v={el:r,x:e,y:a,target:o,send:s,statusesFor:l,title:n,detailSlug:"",filter:""},z(r,e,a),r.textContent="Загрузка…",await Promise.all([_(),$()]),!(!v||v.el!==r)&&E(v)}function z(e,a,o){const s=e.offsetWidth||296,l=e.offsetHeight||320,n=Math.max(8,Math.min(a,window.innerWidth-s-8)),r=Math.max(8,Math.min(o,window.innerHeight-l-8));e.style.left=n+"px",e.style.top=r+"px"}function E(e){const{el:a,send:o,target:s}=e,l=e.statusesFor()||[],n=new Map(l.map(t=>[t.slug,t]));a.innerHTML="";const r=document.createElement("div");r.className="status-palette-head";const d=document.createElement("input");d.type="search",d.placeholder=e.title?`Состояния — ${e.title}`:"Состояние…",d.value=e.filter,d.oninput=()=>{e.filter=d.value,E(e);const t=a.querySelector(".status-palette-head input");t&&(t.focus(),t.setSelectionRange(t.value.length,t.value.length))};const p=document.createElement("button");p.type="button",p.className="status-palette-close",p.innerHTML=N("close",{size:14}),p.title="Закрыть",p.onclick=k,r.append(d,p),a.appendChild(r);const i=e.filter.trim().toLowerCase(),x=(y||[]).filter(t=>!i||[t.name,t.slug,...t.tags||[]].join(" ").toLowerCase().includes(i));if(x.length===0){const t=document.createElement("p");t.className="status-palette-empty",t.textContent=y&&y.length?"Ничего не найдено.":"В этом мире ещё нет состояний — заведи их в конструкторе (Компендиум → Состояния).",a.appendChild(t)}else{const t=document.createElement("div");t.className="status-palette-grid";for(const u of x){const c=u.slug||"",h=n.get(c),g=document.createElement("div");if(g.className="status-cell"+(h?" active":""),u.color&&g.style.setProperty("--cell-color",u.color),g.appendChild(M(u,u.name)),g.title=`${u.name}${c?` (${c})`:""}
